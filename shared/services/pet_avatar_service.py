...
def _run_generation_pipeline(
    ...
    db = SessionLocal()
    try:
        logger.info(f"[Avatar Generation] Starting pipeline for pet_id={pet_id}")
        
        pet = get_pet(db, pet_id, user_id)
        if not pet:
            logger.error(f"[Avatar Generation] Pet not found: pet_id={pet_id}")
            _fail_avatar(db, pet_id, "宠物不存在")
            return

        avatar = db.query(PetAvatar).filter(PetAvatar.pet_id == pet_id).first()
        if not avatar:
            logger.error(f"[Avatar Generation] Avatar record not found: pet_id={pet_id}")
            _fail_avatar(db, pet_id, "Avatar 记录不存在")
            return

        logger.info(f"[Avatar Generation] Found pet and avatar records")
        ...
        try:
            # ---- Step a: 生成基础形象 ----
            logger.info(f"[Avatar Generation] Step 1: Generating base image...")
            ...
            logger.info(f"[Avatar Generation] Calling DashScope API with model={DASHSCOPE_IMAGE_MODEL}")
            logger.info(f"[Avatar Generation] Prompt: {base_prompt[:100]}...")
            ...
            try:
                ...
                logger.info(f"[Avatar Generation] Using direct API call...")
                ...
                if not results:
                    raise RuntimeError("DashScope returned empty results")
                
                img_url = results[0].get("url", "")
                if not img_url:
                    raise RuntimeError("DashScope returned empty URL")
                
                logger.info(f"[Avatar Generation] Got image URL: {img_url[:60]}...")
                
                # 直接使用URL,不下载和上传MinIO
                avatar.base_image_url = img_url
                avatar.status = "done"
                db.commit()
                
                logger.info(f"[Avatar Generation] Base image saved")
                
                # 生成情绪变体(暂时使用相同的URL)
                avatar.emotion_happy_url = img_url
                avatar.emotion_normal_url = img_url
                avatar.emotion_hungry_url = img_url
                avatar.emotion_weak_url = img_url
                db.commit()
                
                logger.info(f"[Avatar Generation] All done for pet_id={pet_id}")
                return
                
            except Exception as api_error:
                logger.error(f"[Avatar Generation] API call failed: {api_error}")
                # 失败时标记为失败状态,不使用占位图降级
                _fail_avatar(db, pet_id, str(api_error))
                return
            ...
            avatar.status = "done"
            avatar.error_message = None
            # 同步更新 PetProfile.avatar_url，让宠物详情接口返回最新头像
            pet.avatar_url = avatar.base_image_url
            db.commit()
