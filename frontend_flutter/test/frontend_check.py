...
用法（在 frontend_flutter/ 目录下执行）:
    python test/frontend_check.py
    python test/frontend_check.py --fix
...
def main():
    ...
    # 默认前端目录 = 脚本自身所在 test/ 的上级（frontend_flutter/），迁移仓库不失效
    default_cwd = str(Path(__file__).resolve().parent.parent)
    parser.add_argument("--cwd", default=default_cwd, help="前端目录")
