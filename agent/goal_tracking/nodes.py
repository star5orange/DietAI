...
def calculate_fasting_targets_node(state: GoalTrackingState) -> GoalTrackingState:
    ...
    与 /api/goals/daily-status 共用 apply_fasting_target_adjustment，
    断食日语义与前端一致（1=周一）。
    """
    try:
        ...
        base_targets = dict(state.get("macro_targets") or {})
        base_targets.setdefault("calories", state.get("daily_calorie_target", 2000))

        db = SessionLocal()
        try:
            adjusted = apply_fasting_target_adjustment(
                db, user_id, base_targets
            )
        finally:
            db.close()

        # 仅在断食计划实际调整（出现 note）时覆盖目标
        if adjusted.get("note"):
            state["daily_calorie_target"] = adjusted["calories"]
            state["macro_targets"] = adjusted
            state["current_step"] = "fasting_targets_adjusted"
            logger.info(
                f"Adjusted fasting targets for user {user_id}: "
                f"target={adjusted['calories']} kcal, note={adjusted.get('note')}"
            )
